...
This is a **core pattern — included in every Astro project**. It builds on [Portless Dev Workflow](./astro-portless.md).
...
```bash
SITE_URL=https://vetoxzyn.localhost
```

(This project has no backend contract — no `API_BASE_URL`/`API_TOKEN`.)
...
> Vetoxzyn exception: this project does NOT use the plugin. No `worktree_create` / `worktree_delete`, no `.opencode/worktree.jsonc`, no new terminal — manual siblings in the same session only (see `AGENTS.md` § Git worktrees).
...
Vetoxzyn ships no `.opencode/worktree.jsonc` (deleted intentionally — plugin-only config, unused here).

`.opencode` skills/commands sync (openspec in the sibling — markdown only, no plugin runtime):

```bash
mkdir -p ../<project>-<branch>/.opencode/skills ../<project>-<branch>/.opencode/commands
cp -rn .opencode/skills/openspec-* ../<project>-<branch>/.opencode/skills/
cp -rn .opencode/commands/opsx-*.md ../<project>-<branch>/.opencode/commands/
```
...
|---|---|---|
| `node_modules/` | gitignored | `pnpm install` per checkout (no symlink in vetoxzyn) |
| `.env` | gitignored (secrets) | copy from main checkout or `.env.example` |
...
- Deleting a worktree does **not** stop its dev server — stop it first (Ctrl+C), then remove. Agent-orphaned servers (proxy 404, direct port alive) → see "Running under AI agents" in [Portless Dev Workflow](./astro-portless.md).
...
## Appendix: opencode-worktree Plugin (Agent Sessions — NOT used in vetoxzyn)

> Vetoxzyn does not use this plugin. The text below is kept as reference for other projects only.
