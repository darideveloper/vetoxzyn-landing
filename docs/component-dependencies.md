...
├── organisms/Testimonials.astro (static, id="testimonios", no client: directive)
...
└── organisms/ContactSection.astro (static shell + backdrop + grid, section#contacto)
...
├── Input.tsx     (F1 underline, store-bound, optional idPrefix → label-associated id) ──► store/useField ──► store/contact
├── Textarea.tsx  (F3 glass, store-bound, optional idPrefix → label-associated id) ──► store/useField ──► store/contact
├── Checkbox.tsx  (F2 pill, store-bound, optional idPrefix → label-associated id) ──► store/useField ──► store/contact
