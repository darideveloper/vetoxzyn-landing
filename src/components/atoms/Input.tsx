...
export function Input({ field, useField = defaultUseField, label, idPrefix = "", className, ...props }: InputProps) {
  ...
  return (
    <div className="flex flex-col gap-2 p-2">
      ...
      <input
        className={cn(
          "w-full border-0 border-b-2 border-on-surface/10 bg-transparent px-0 py-3 text-lg transition-colors duration-[var(--duration-hover)] ease-[var(--ease-hover)] hover:border-on-surface/30",
