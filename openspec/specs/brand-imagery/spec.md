...
Single local AI master per image slot with per-slot responsive widths served via `astro:assets Image`; no `-384`/`-512` variant sprawl, no per-slot `README` stubs, and no external hotlinks except the 3 kept Unsplash avatars.
## Requirements
