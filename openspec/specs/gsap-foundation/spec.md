...
Shared, SSR-safe GSAP foundation for Astro: a single `src/lib/gsap.ts` module every animated component imports from, the ClientRouter View-Transition lifecycle, the SEO-safe `.js-reveal`/`.no-js` reveal base, the reduced-motion contract, and token/palette + interaction-feedback compliance.
