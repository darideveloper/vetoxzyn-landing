...
- No API endpoint modules, no SSR/hybrid output (rely on Astro default `static`, no explicit `output` key), no PWA (service worker, manifest, offline page).
- No GSAP/animation system, no content collections, no auth/session store.
- Core Web Vitals extras deferred except `inlineStylesheets: "always"`: no custom font preloads, no third-party preconnects, no hero-image `<link rel="preload">` — revisit when real fonts/embeds land.
...
No explicit `output` key (Astro default `static`). nginx: gzip, security headers, `/_astro/*` immutable 1y, HTML `no-cache`, `try_files $uri $uri/index.html =404`. PWA blocks (offline error_page, sw no-cache, manifest) deleted per doc instructions. No `PUBLIC_*` var exists yet so the Dockerfile ships zero `ARG/ENV` pairs; the dependency layer copies `pnpm-workspace.yaml` alongside `package.json` + lockfile (required — this repo uses workspaces).

**7. Portless named `vetoxzyn`.**
`dev` script: `portless vetoxzyn pnpm astro dev`; `astro.config.mjs` reads `process.env.PORT ?? 4321` with `strictPort: true` (fail fast if taken); `.env` holds server-only `SITE_URL=https://vetoxzyn.localhost` (read via `process.env`, never `import.meta.env`). Global `portless` install, not a dependency.
